...
export function Menu({ isOpen }: MenuProps) {
    ...
    return (
        <ScrollArea className="[&>div>div[style]]:!block min-h-full rounded-md">
            <nav className="mb-8 h-full w-full">
                <ul className="flex flex-col min-h-[calc(100vh-48px-36px-16px-32px)] lg:min-h-[calc(100vh-32px-40px-32px)] items-start space-y-1 px-2">
                    {menuList.map(({ groupLabel, menus }, index) => (
                        <li className={cn("w-full list-none", groupLabel ? "pt-3" : "")} key={index}>
                            {(isOpen && groupLabel) || isOpen === undefined ? (
                                <p className="text-sm font-medium text-gray-500 px-4 pb-0 max-w-[248px] truncate">
                                    {groupLabel}
                                </p>
